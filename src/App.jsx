...
.auth-card{background:var(--bg2);border:1px solid var(--border);border-radius:20px;padding:40px;width:100%;max-width:420px}
...
.history-item{background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;cursor:pointer}
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="form-group"><label className="form-label">Description</label><textarea className="form-textarea" value={form.description} onChange={e=>set("description",e.target.value)} placeholder="Contenu détaillé de la séance..."/></div>
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="grid-2"><div className="form-group"><label className="form-label">Distance réelle (km)</label><input className="form-input" type="number" step="0.1" value={dist} onChange={e=>setDist(e.target.value)}/></div><div className="form-group"><label className="form-label">Durée réelle (min)</label><input className="form-input" type="number" value={dur} onChange={e=>setDur(e.target.value)}/></div></div>
            ...
            <div key={s.id}>
              <div className="history-item" onClick={()=>setSelected(selected?.id===s.id?null:s)}>
                <div className="history-left">
                  ...
                  <div className="history-info"><div className="title">{s.title}</div><div className="sub">{s.type}{s.distance?` · ${s.actual_distance||s.distance}km`:""}{s.denivele?` · D+${s.denivele}m`:""}</div></div>
                </div>
    ...
    <div>
      ...
      {showForm&&(
        <div className="card mb-24">
          ...
          <div className="grid-2"><div className="form-group"><label className="form-label">Nom *</label><input className="form-input" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Ex: Diagonale des Fous..."/></div><div className="form-group"><label className="form-label">Date *</label><input className="form-input" type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))}/></div></div>
          <div className="grid-2"><div className="form-group"><label className="form-label">Distance (km)</label><input className="form-input" type="number" value={form.distance} onChange={e=>setForm(f=>({...f,distance:e.target.value}))}/></div><div className="form-group"><label className="form-label">D+ (m)</label><input className="form-input" type="number" value={form.denivele} onChange={e=>setForm(f=>({...f,denivele:e.target.value}))}/></div></div>
          <div className="form-group"><label className="form-label">Notes / Objectif de temps</label><textarea className="form-textarea" value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Ex: Finisher, objectif 8h..."/></div>
      ...
      {next&&(
        <div className="race-card">
          ...
          <div style={{fontSize:13,color:"var(--muted)",marginTop:4}}>{formatDate(next.date)}{next.distance?` · ${next.distance}km`:""}{next.denivele?` · D+${next.denivele}m`:""}</div>
      ...
      {upcoming.length>1&&<div className="mb-24"><div className="section-title">Autres objectifs</div>{upcoming.slice(1).map(r=><div key={r.id} className="card-sm mb-16" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontWeight:600}}>{r.name}</div><div className="text-muted">{formatDate(r.date)} · J-{daysUntil(r.date)}</div></div><button className="btn btn-danger btn-sm" onClick={()=>deleteRace(r.id)}>✕</button></div>)}</div>}
          ...
          <div>
            ...
            <div className="card mb-24" style={{background:"rgba(212,255,71,.05)",border:"1px solid rgba(212,255,71,.2)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:16}}>
                ...
                <div style={{fontSize:13,color:"var(--muted)"}}>Partage ce code à tes athlètes.<br/>Ils l'entreront lors de leur inscription sur l'app.</div>
              </div>
            </div>
            <div className="section-title">Liste des athlètes</div>
            {athletes.length===0?<div className="card" style={{textAlign:"center",padding:40,color:"var(--muted)"}}>Aucun athlète pour l'instant.<br/><span style={{fontSize:13}}>Partage ton code coach pour qu'ils s'inscrivent.</span></div>:(
